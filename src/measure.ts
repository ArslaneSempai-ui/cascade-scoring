...
export function rapportMd(m: MesurePublique, recommandee: CellulePlacee | null): string {
  const l: string[] = [
    ...
    + (m.paliers.absents.length
      ? ` **Not in tonight's registry: ${m.paliers.absents.map((p) => `\`${p}\``).join(", ")}**. Measured when it ships, and shown absent until then.`
    ...
    recommandee
      ...
      : `No cell holds a recall lower bound of ${(ASSUMPTIONS.recallFloor * 100).toFixed(0)} % on the written dossiers, and the report says so.`,
    ``,
...
async function principal(): Promise<void> {
  ...
  console.log(recommandee
    ...
    : `no cell holds the recall floor of ${ASSUMPTIONS.recallFloor} at the lower bound, and the report says so.`);
}
